...
def run(opt_type: str, plot: bool, params_file: str):
    ...
    plot_opts = {
        ...
        "raceline": plot,  # plot optimized path
        "imported_bounds": plot,  # plot imported bounds (analyze difference to interpolated bounds)
        "raceline_curv": plot,  # plot curvature profile of optimized path
        "racetraj_vel": plot,  # plot velocity profile
    ...
    mintime_opts = {"tpadata": None, "warm_start": False, "var_friction": None, "reopt_mintime_solution": False, "recalc_vel_profile_by_tph": False}
    ...
    if opt_type == "mintime" and not mintime_opts["recalc_vel_profile_by_tph"] and lap_time_mat_opts["use_lap_time_mat"]:
        raise IOError("Lap time calculation table should be created but velocity profile recalculation with TPH solver is" " not allowed!")
    ...
    file_paths["tpamap"] = os.path.join(file_paths["module"], "inputs", "frictionmaps", file_paths["track_name"] + "_tpamap.csv")

    if mintime_opts["tpadata"] is None:
        file_paths["tpadata"] = os.path.join(file_paths["module"], "inputs", "frictionmaps", file_paths["track_name"] + "_tpadata.json")
    else:
    ...
    if (
        ...
        print("WARNING: var_friction option is not None but friction map data is missing for current track -> Setting" " var_friction to None!")
    ...
    if opt_type == "shortest_path":
        ...
        pars["vehicle_params_mintime"]["wheelbase"] = (
            pars["vehicle_params_mintime"]["wheelbase_front"] + pars["vehicle_params_mintime"]["wheelbase_rear"]
        )
    ...
    reftrack_imp = helper_funcs_glob.src.import_track.import_track(
        imp_opts=imp_opts, file_path=file_paths["track_file"], width_veh=pars["veh_params"]["width"]
    )

    # import ggv and ax_max_machines (if required)
    if not (opt_type == "mintime" and not mintime_opts["recalc_vel_profile_by_tph"]):
        ggv, ax_max_machines = tph.import_veh_dyn_info.import_veh_dyn_info(
            ggv_import_path=file_paths["ggv_file"], ax_max_machines_import_path=file_paths["ax_max_machines_file"]
        )
    else:
        ggv = None
    ...
    if (
        ...
        and (pars["optim_opts"]["ax_pos_safe"] is None or pars["optim_opts"]["ax_neg_safe"] is None or pars["optim_opts"]["ay_safe"] is None)
    ):
    ...
    reftrack_interp, normvec_normalized_interp, a_interp, coeffs_x_interp, coeffs_y_interp = helper_funcs_glob.src.prep_track.prep_track(
    ...
    if opt_type == "mincurv":
        ...
        alpha_opt = tph.opt_shortest_path.opt_shortest_path(
            reftrack=reftrack_interp, normvectors=normvec_normalized_interp, w_veh=pars["optim_opts"]["width_opt"], print_debug=debug
        )

    elif opt_type == "mintime":
        # reftrack_interp, a_interp and normvec_normalized_interp are returned for the case that non-regular sampling was
        # applied
        alpha_opt, v_opt, reftrack_interp, a_interp_tmp, normvec_normalized_interp = opt_mintime_traj.src.opt_mintime.opt_mintime(
            reftrack=reftrack_interp,
    ...
    if opt_type == "mintime" and mintime_opts["reopt_mintime_solution"]:
        ...
        reftrack_interp, normvec_normalized_interp, a_interp = helper_funcs_glob.src.prep_track.prep_track(
        ...
        if debug:
            ...
            bound_r_reopt = reftrack_interp[:, :2] + np.expand_dims(reftrack_interp[:, 2], axis=1) * normvec_normalized_interp
            bound_l_reopt = reftrack_interp[:, :2] - np.expand_dims(reftrack_interp[:, 3], axis=1) * normvec_normalized_interp
            ...
            print(
                ...
                % (np.amin(d_r_reopt) - pars["veh_params"]["width"] / 2, np.amin(d_l_reopt) - pars["veh_params"]["width"] / 2)
            )
    ...
    psi_vel_opt, kappa_opt = tph.calc_head_curv_an.calc_head_curv_an(
        coeffs_x=coeffs_x_opt, coeffs_y=coeffs_y_opt, ind_spls=spline_inds_opt_interp, t_spls=t_vals_opt_interp
    )
    ...
    ax_profile_opt = tph.calc_ax_profile.calc_ax_profile(vx_profile=vx_profile_opt_cl, el_lengths=el_lengths_opt_interp, eq_length_output=False)

    # calculate laptime
    t_profile_cl = tph.calc_t_profile.calc_t_profile(vx_profile=vx_profile_opt, ax_profile=ax_profile_opt, el_lengths=el_lengths_opt_interp)
    ...
    if lap_time_mat_opts["use_lap_time_mat"]:
        ...
        ggv_scales = np.linspace(
            ...
            int((lap_time_mat_opts["gg_scale_range"][1] - lap_time_mat_opts["gg_scale_range"][0]) / lap_time_mat_opts["gg_scale_stepsize"]) + 1,
        )
        top_speeds = np.linspace(
            lap_time_mat_opts["top_speed_range"][0] / 3.6,
            lap_time_mat_opts["top_speed_range"][1] / 3.6,
            int((lap_time_mat_opts["top_speed_range"][1] - lap_time_mat_opts["top_speed_range"][0]) / lap_time_mat_opts["top_speed_stepsize"]) + 1,
        )
        ...
        for i, top_speed in enumerate(top_speeds):
            for j, ggv_scale in enumerate(ggv_scales):
                tph.progressbar.progressbar(i * ggv_scales.shape[0] + j, top_speeds.shape[0] * ggv_scales.shape[0], prefix="Simulating laptimes ")
                ...
                ax_profile_opt = tph.calc_ax_profile.calc_ax_profile(
                    vx_profile=vx_profile_opt_cl, el_lengths=el_lengths_opt_interp, eq_length_output=False
                )

                # calculate lap time
                t_profile_cl = tph.calc_t_profile.calc_t_profile(
                    vx_profile=vx_profile_opt, ax_profile=ax_profile_opt, el_lengths=el_lengths_opt_interp
                )
    ...
    if plot_opts["imported_bounds"]:
        ...
        _, _, _, normvec_imp = tph.calc_splines.calc_splines(path=np.vstack((reftrack_imp[::n_skip, 0:2], reftrack_imp[0, 0:2])))
